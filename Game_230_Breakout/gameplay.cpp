...
void gameplay::soundInit() {
	...
	if (!hitPaddleSoundBuffer.loadFromFile("hitPaddle.wav"))
		return;
	hitPaddle.setBuffer(hitPaddleSoundBuffer);
	hitPaddle.setVolume(50);

	if (!loseLifeSoundBuffer.loadFromFile("loseLife.wav"))
		return;
	loseLife.setBuffer(loseLifeSoundBuffer);
	loseLife.setVolume(50);

	if (!damageBrickSoundBuffer.loadFromFile("damageBrick.wav"))
		return;
	damageBrick.setBuffer(damageBrickSoundBuffer);
	damageBrick.setVolume(50);

	if (!destroyBrickSoundBuffer.loadFromFile("destroyBrick.wav"))
		return;
	destroyBrick.setBuffer(destroyBrickSoundBuffer);
	destroyBrick.setVolume(50);

	if (!gameOverSoundBuffer.loadFromFile("gameOver.wav"))
		return;
	gameOver.setBuffer(gameOverSoundBuffer);
